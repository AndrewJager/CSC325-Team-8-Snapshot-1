...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addRoleOption(option => option.setName(`role4`).setDescription(`Third role you want to see set up`).setRequired(true))
		.addRoleOption(option => option.setName(`role5`).setDescription(`Third role you want to see set up`).setRequired(true)),
	...
	async execute(interaction, client) {
		...
		const embed = new EmbedBuilder()
            ...
            .setDescription('Select a course you are registered to.')
			.setColor('Black');
		...
		collector.on('collect', async (i) => {
			...
			if (i.guild.members.me.roles.highest.position < role1.position) {
				i.update({ content: 'My role is below the roles I am trying to mess with.', ephemeral:true });
				return;
			}
